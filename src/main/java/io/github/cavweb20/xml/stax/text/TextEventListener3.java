...
public class TextEventListener3
{
    // Setting up the logging properties
    private static final Logger LOG = LoggerFactory.getLogger(TextEventListener3.class);
        ...
        try
        {
            if (args.length < 1)
            {
                if (LOG.isInfoEnabled())
                    LOG.info("Usage: java TextEventListener3 URL");
                if (LOG.isDebugEnabled())
                    LOG.debug("##### End #####");
                return;
            }
