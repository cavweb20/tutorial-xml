...
public class XLinkLinkFinder
{
    // Setting up the logging properties
    private static Logger LOG = LoggerFactory.getLogger(XLinkLinkFinder.class);
    
    private static String printLinks (XMLStreamReader parser)
    {
        return parser.getAttributeValue("http://www.w3.org/1999/xlink", "href");
    }

    /**
     * Example URL: http://www.rddl.org/
     * @param args
     */
        ...
        try
        {
            if (args.length < 1)
            {
                if (LOG.isInfoEnabled())
                    LOG.info("Usage: java XLinkLinkFinder URL");
                if (LOG.isDebugEnabled())
                    LOG.debug("##### End #####");
                return;
            }
